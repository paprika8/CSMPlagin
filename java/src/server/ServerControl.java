...
public class ServerControl implements ApplicationListener{
    ...
    public Cons<GameOverEvent> gameOverListener = event -> {
        ...
        if(nextMapOverride != null){
            ...
            map = maps.getNextMap(lastMode, state.map);
        if(map != null){
    ...
    protected void setup(String[] args){
        ...
        Log.logger = (level1, text) -> {
            ...
            String result = bold + lightBlack + "[" + dateTime.format(LocalDateTime.now()) + "] " + reset +  Log.format(tags[level1.ordinal()] + " " + text + "&fr");
            System.out.println(result);

            if(Config.logging.bool()){
                logToFile("[" + dateTime.format(LocalDateTime.now()) + "] " +  Log.formatColors(tags[level1.ordinal()] + " " + text + "&fr", false));
            }

            if(socketOutput != null){
                try{
                    socketOutput.println( Log.formatColors(text + "&fr", false));
                }catch(Throwable e1){
    ...
    public void logToFile(String text){
        server.LogManager.logToFile(text, this);
    }
    ...
    public static void info(String text, Object... args) {
        Log.info(LogManager.translate(text), args);
    }
